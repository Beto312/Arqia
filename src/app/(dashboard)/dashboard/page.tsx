...
export default async function DashboardPage() {
  ...
  const statusColors: Record<string, string> = {
    ...
    EXECUTIVE_PROJECT: "bg-primary-100 text-primary-700",
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 mb-10">
        <Link
          ...
          className="flex items-center gap-4 p-5 bg-primary-600 hover:bg-primary-700 rounded-2xl text-white transition-colors group"
        >
          <Sparkles className="h-8 w-8" />
          <div>
            <div className="font-semibold">Novo projeto com IA</div>
            <div className="text-primary-200 text-sm">Descreva e gere em minutos</div>
          </div>
          <ArrowRight className="h-5 w-5 ml-auto group-hover:translate-x-1 transition-transform" />
        </Link>
        <Link
          href="/dashboard/projects"
          className="flex items-center gap-4 p-5 bg-surface border border-border hover:border-zinc-300 dark:hover:border-zinc-600 rounded-2xl text-zinc-900 dark:text-zinc-100 transition-colors group"
        >
      ...
      <div>
        ...
        {projects.length === 0 ? (
          <div className="text-center py-16 bg-surface rounded-2xl border border-border">
            ...
            <Link
              href="/dashboard/generate"
              className="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-medium px-5 py-2.5 rounded-lg transition-colors text-sm"
            >
          ...
          <div className="space-y-3">
            {projects.map((p: (typeof projects)[number]) => (
              <Link
                ...
                className="flex items-center gap-4 p-4 bg-surface border border-border rounded-xl hover:border-zinc-200 dark:hover:border-zinc-700 transition-colors"
              >
                <div className="h-10 w-10 bg-primary-100 dark:bg-primary-950/40 rounded-lg flex items-center justify-center">
